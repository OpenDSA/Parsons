.. parsonsprob:: pb-Clique2IS-DAG
  ...
  =====
  b. \(E'\) is the complement of \(E\) that is \(G'\) has all the edges that is \(\textbf{not}\) present in \(G\). #tag:step2; depends:step1;
  =====
  ...
  =====
  If there is an independent set of size \(k\) in the complement graph \(G'\), it implies no two vertices share an edge in \(G'\), which further implies all of those vertices share an edge with all others in \(G\) forming a clique. That is, \(\textbf{there exists a clique of size}\ k\ \textbf{in}\ G\). #tag:if; depends:iff;
  =====
  If there is a clique of size \(k\) in the graph \(G\), it implies all vertices share an edge with all others in \(G\), which further implies no two of these vertices share an edge in \(G'\) (thus forming an Independent Set). That is, \(\textbf{there exists an independent set of size}\ k\ \textbf{in}\ G'\). #tag:onlyif; depends:iff;
